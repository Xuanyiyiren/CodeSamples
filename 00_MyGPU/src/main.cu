...
int main() {
  ...
  */
  ...
  std::cout << "Compute capability: " << props.major << "." << props.minor
            << std::endl;
  std::cout << "Memory: " << props.totalGlobalMem / float(1 << 30) << " GiB"
            << std::endl;
  std::cout << "Multiprocessors: " << props.multiProcessorCount << std::endl;

  // Clock rate is no longer in cudaDeviceProp, use cudaDeviceGetAttribute
  int clockRateKHz = 0;
  cudaDeviceGetAttribute(&clockRateKHz, cudaDevAttrClockRate, device);
  std::cout << "Clock rate: " << clockRateKHz / 1'000'000.0f << " GHz"
            << std::endl;
...
/*
Exercises:
1) Change the behavior such that the properties are not just printed for one,
but all available CUDA devices you have! (Even if you have just one) 2) Print a
few more interesting properties and read up in the specification what they mean.
*/
